  ...
  const fetchEarnings = async (page: number) => {
    ...
    try {
      const response = await apiCall<{
        ...
        totalPages: number;
      }>(`/api/users/earnings/all?page=${page}&limit=20`);
      setEarnings(response.earnings || []);
      setTotalPages(response.totalPages || 1);
    } catch (err: any) {
      setError(err.message || "Failed to retrieve system-wide payouts.");
    } finally {
      setIsLoading(false);
    }
  };

  const triggerDeleteEarning = (id: string, username: string, amount: number, symbol: string) => {
    setToastConfirm({
      show: true,
      id,
      message: `Are you absolutely sure you want to delete this yield earning of $${amount.toFixed(2)} (${symbol}) credited to user "${username}"? This action is permanent and cannot be undone.`
    });
  };
    ...
    <div className="p-[10px] md:p-8 flex flex-col gap-6 w-full text-left min-h-screen pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-900 pb-5">
        <div className="flex flex-col gap-1">
          <h3 className="text-2xl font-black text-white tracking-tight uppercase">System Earnings Ledger</h3>
          <p className="text-sm text-neutral-400 font-light">
            Monitor, audit, and track live compound interest payouts generated by active deposits across all user accounts.
          </p>
        </div>
      ...
      <div className="bg-[#0f1115] border border-neutral-900 rounded-lg overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8">
            ...
            <span className="text-sm text-white font-black uppercase tracking-wider">No Compound Yields Generated Yet</span>
            <span className="text-xs text-neutral-500 mt-2">Active deposits automatically dispatch returns every 24 hours.</span>
          </div>
        ) : (
          <>
                  ...
                  {paginatedEarnings.map((e) => (
                    <tr key={e._id} className="hover:bg-neutral-900/20 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <span className="text-sm font-extrabold text-white">{e.username}</span>
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-black text-[#e4c126] uppercase tracking-wider">{e.planPercent}% Daily Rate</span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 text-right">
                        <button
                          onClick={() => triggerDeleteEarning(e._id, e.username, e.earning, e.currencySymbol)}
